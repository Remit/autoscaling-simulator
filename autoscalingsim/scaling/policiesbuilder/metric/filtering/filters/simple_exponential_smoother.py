...
class SimpleExponentialSmoother(ValuesFilter):
    ...
    def __init__(self, config : dict):
        ...
        self.smoothing_level = ErrorChecker.key_check_and_load('smoothing_level', config, self.__class__.__name__, default = 0.2)

    def filter(self, values : pd.DataFrame):

        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            values.value = SimpleExpSmoothing(values.interpolate().fillna(method = 'backfill')).fit(smoothing_level = self.smoothing_level).fittedvalues
