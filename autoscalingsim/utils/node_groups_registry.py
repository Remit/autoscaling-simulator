...
class NodeGroupsRegistry:
    ...
    def register_node_group_for_service(self, node_group : 'HomogeneousNodeGroup', service_name : str):
    ...
    def register_node_group(self, node_group : 'HomogeneousNodeGroup'):

        for service_name in node_group.running_services:
            self.register_node_group_for_service(node_group, service_name)

    def deregister_node_group(self, node_group : 'HomogeneousNodeGroup'):

        for service_name in self._Node_groups_by_service.keys():
            self.deregister_node_group_for_service(node_group, service_name)

    def block_for_scheduling(self, node_group : 'HomogeneousNodeGroup'):

        self._unschedulable_node_groups.append(node_group.id)

    def deregister_node_group_for_service(self, node_group : 'HomogeneousNodeGroup', service_name : str):
